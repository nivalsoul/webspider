...
public class Gongzhonghao {
	static Dao dao = null;
	static String[] types = {"热门", "推荐", "段子手", "养生堂", "私房话", "八卦精",
		"爱生活", "财经迷", "汽车迷", "科技咖", "潮人帮", "辣妈帮", "点赞党", "旅行家",
		"职场人", "美食家", "古今通", "学霸族", "星座控", "体育迷"};
	...
	private static void getPage(String url, int k) throws Exception {
		...
		Elements users = doc.select("div.pos-wxrw");
		Elements articles = doc.select("div.wx-news-info2");
		System.out.println("访问："+url);
		System.out.println("文章数："+articles.size());
		for(int i=0;i<users.size();i++){
			Element user = users.get(i);
			Element article = articles.get(i);
			String author = user.child(0).select("p").get(1).text();
